...
values.missing <- apply(na, 2, sum)
values.present <- dt.final[,.N] - apply(na, 2, sum)
...
fwrite(dt.values.present,
       paste0(dir.analysis, prefix.files, "Values-Present.csv"))

fwrite(dt.values.missing,
       paste0(dir.analysis, prefix.files, "Values-Missing.csv"))
...
```{r}
print(dt.values.missing[missing > 0])
```

## Present Values

```{r}
print(dt.values.present)
