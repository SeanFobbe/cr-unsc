...
freqtable.year <- f.fast.freqtable(dt.final,
                                 varlist = "year",
                                 sumrow = TRUE,
                                 output.list = TRUE,
                                 output.kable = FALSE,
...
freqtable.year.nosum <- freqtable.year[-.N]

```


```{r, CR-UNSC_05_Barplot_Resolutions-by-Year, fig.height = 6, fig.width = 9}
ggplot(data = freqtable.year.nosum) +
    geom_bar(aes(x = reorder(year,
                             -N),
                 y = N),
             stat = "identity",
             fill = "black",
