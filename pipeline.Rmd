...
list(
    tar_target(csv.record.voting,
               "data/UNSC_Record_Voting_2636.csv",
               format = "file"),
...
### Debugging Resolution Numbers

```{targets tar.download.debugnums}
tar_target(debug.nums,
           sample(1:config$download$resmax, config$debug$sample))
```
...
tar_target(dt.record.final,
           f.record_main(recordtable.stable = record.table.stable,
                          limit = config$download$resmax,
                          debug.toggle = config$debug$toggle,
                          debug.nums = debug.nums))
```
...
tar_target(html.record.voting,
                f.download(url = dt.record.voting.final$url_record,
                           filename = paste0(dt.record.voting.final$res_no, ".html"),
...
values <- tibble::tibble(
                      ...
                      url = c(quote(dt.download.final$url_res_ar),
                              ...
                              quote(dt.download.final$url_draft_en),
                              quote(dt.download.final$url_meeting_en)),
                      filename = c(quote(paste0(dt.download.final$doc_id, "_AR.pdf")),
                                   ...
                                   quote(paste0(dt.download.final$doc_id, "_Draft_EN.pdf")),
                                   quote(paste0(dt.download.final$doc_id, "_MeetingRec_EN.pdf"))),
...
<!--     dt_ocr_all, -->
<!--     tarchetypes::tar_select_targets(zip.list, starts_with("dt_ocr")) -->
<!-- ) -->
...
### Combine Resolution Text Table Targets


```{targets tar.download.dt.res}
tar_target(dt_res_all,
           rbind(dt_res_ar,
                 dt_res_en,
                 dt_res_es,
                 dt_res_fr,
...
### Combine Draft and Meeting Text Table Targets


```{targets tar.download.dt.meetingdraft}
tar_target(dt_draft_meeting_all,
           rbind(dt_draft_en,
                 dt_ocr_draft_en,
                 dt_meeting_en,
                 dt_ocr_meeting_en,
...
<!--                 f.tar_pdf_extract(x = pdf_res_en, -->
...
<!--                 f.tar_pdf_extract(x = pdf_draft_en, -->
...
<!--                 f.tar_pdf_extract(x = pdf_meeting_en, -->
...
tar_target(dt.intermediate,
           f.merge_data(dt.res.en = dt_res_en,
                        dt.res.en.gold = dt_res_en_gold,
                        dt.draft.en = dt_draft_en,
                        dt.meeting.en = dt_meeting_en,
...
### Create Variables with REGEX from Text
...
<!--                 f.tar_zip(csv_final, -->
<!--                               filename = gsub("\\.csv", "\\.zip", basename(csv_final)), -->
<!--                               dir = "output", -->
<!--                               mode = "cherry-pick"), -->
<!--                 format = "file") -->
<!-- ``` -->
...
<!--                 f.tar_zip(csv_meta, -->
<!--                               filename = gsub("\\.csv", "\\.zip", basename(csv_meta)), -->
<!--                               dir = "output", -->
<!--                               mode = "cherry-pick"), -->
<!--                 format = "file") -->
<!-- ``` -->
