...
tar_target(html.record.draft,
                f.download(url = dt.download[!is.na(url_record_draft)]$url_record_draft,
                           filename = paste0(dt.download[!is.na(url_record_draft)]$res_no, ".html"),
                           dir = "record_draft",
                           sleep.min = 0.5,
                           sleep.max = 1,
                           retries = 3,
...
tar_target(html.record.meeting,
                f.download(url = dt.download[!is.na(url_record_meeting)]$url_record_meeting,
                           filename = paste0(dt.download[!is.na(url_record_meeting)]$res_no, ".html"),
                           dir = "record_meeting",
                           sleep.min = 1,
                           sleep.max = 1.5,
                           retries = 3,
...
tar_target(var_constants,
           data.frame(version = as.character(datestamp),
                      ...
                      lizenz = as.character(config$license$data))[rep(1, nrow(dt.res.en)),])
```
...
tar_target(vars_additional,
           data.table(var_lingstats,
                      var_constants))

```


### Merge Additional Variables with Main Data Set

```{targets tar.enhance11}
tar_target(dt.res.final,
           cbind(dt.res.en,
                 vars_additional))
...
tar_target(lingstats.summary,
                f.lingstats_summary(dt.res.final,
                                    germanvars = FALSE))
